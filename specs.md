...
- Use crcglot's generated crc16-xmodem, vendored into the wheel (`src/crclink/crc16_xmodem.py`); do not hand-roll or hardcode the CRC. crcglot owns the algorithm and its parameters and generates the code.
- crcglot is a build-time code generator (a dev dependency, pinned `crcglot>=0.22.0`), so the wheel has no runtime dependencies.
- Regenerate the vendored module from the pinned crcglot when you bump it; never hand-edit it.
